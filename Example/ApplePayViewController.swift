...
class ApplePayViewController: UIViewController {
    ...
    func setupMerchantConfiguration() {
        ...
        PaysafeSDK.applePayMerchantConfiguration = PaysafeSDK.ApplePayMerchantConfiguration(applePayMerchantId: "merchant.com.paysafe.documentation",
                                                                                            countryCode: "US",
                                                                                            currencyCode: "USD")
    }
    ...
    private func didReceiveResultFromAppleService(_ result: Result<PKPayment, Error>) {
        ...
        case let .success(payment):
            storeToBackend(payment)
        }
    }

    private func storeToBackend(_ payment: PKPayment) {
        guard let applePayInfo = createTokenWithPayment(payment) else {
            displayAlert(title: nil, message: Constants.simulatorNotSupportedMessage)
            return
        }
        ...
        merchantBackend.sendApplePay(applePayInfo) { [weak self] applePayResult in
            ...
            case .success(let info):
                self?.displayAlert(title: Constants.successfulTransactionTitle, message: Constants.successfulTransactionMessage + info.id)
            }
        }
    }
}
